...
import { AgentResult, IssueCategory, SeverityLevel, Issue, IssueStatus, ReportInput, GeoLocation, Prediction } from '@/types';
...
export abstract class BaseAgent {
  ...
  abstract run(ctx: AgentContext): Promise<{ data: Record<string, any>; confidence: number; reasoning?: string }>;
}
...
export class CategoryAgent extends BaseAgent {
  ...
  async run(ctx: AgentContext) {
    ...
    return {
      data: { category },
      confidence: reportOut?.confidence ?? 0.85,
...
export class SeverityAgent extends BaseAgent {
  ...
  async run(ctx: AgentContext) {
    ...
    return {
      ...
      confidence: reportOut?.confidence ?? 0.88,
