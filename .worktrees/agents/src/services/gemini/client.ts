...
export async function compareImages(beforeImage: string, afterImage: string): Promise<{ status: 'Fixed' | 'Partially Fixed' | 'Not Fixed'; confidence: number; explanation: string }> {
  if (!GEMINI_API_KEY) {
...
export async function predictHotspots(historicalData: any[]): Promise<any[]> {
  if (!GEMINI_API_KEY) {
    return [];
  }
  try {
